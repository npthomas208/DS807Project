...
gam_red <- gam(r.value~s(x,df=rdf)+s(y,df=rdf),data=rgbImage)
gam_green <- gam(g.value~s(x,df=gdf)+s(y,df=gdf),data=rgbImage)
gam_blue <- gam(b.value~s(x,df=bdf)+s(y,df=bdf),data=rgbImage)
