...
for(i in 1:length(nn$csv$file)){
  nn$csv$image[i] = list(as.integer(outer(readJPEG(paste("./Images/Test/", nn$csv$file[i], sep="")),  255)))
}

nn$tarray <- function(x) aperm(x, seq_along(dim(x)))
nn$test$x = nn$tarray(array(unlist(nn$csv$image), c(length(nn$csv$image),1024, 1024, 3)))
...
p_array <- par(mfrow=c(3,4))
p_ <- rep(NA, 12)

i=1
for (i in 1:12){
  rgbImage_ <- data.frame(
  x=rep(1:1024, each=1024),
  y=rep(1024:1, 1024),
  r.value=rescale(as.vector(nn$train$x[i,,,1]), to=1, from=0),
  g.value=rescale(as.vector(nn$train$x[i,,,2]), to=1, from=0),
  b.value=rescale(as.vector(nn$train$x[i,,,3]), to=1, from=0))

  p_[i] <- ggplot(data=rgbImage_, aes(x=x, y=y, col=rgb(r.value,g.value,b.value))) +
    ggtitle("bluejays")+
    geom_point() + 
    scale_color_identity()
