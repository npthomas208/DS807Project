...
gam_red <- gam(c(r.value)~s(x,df=rdf)+s(y,df=rdf),data=rgbImage)
gam_green <- gam(c(g.value)~s(x,df=gdf)+s(y,df=gdf),data=rgbImage)
gam_blue <- gam(c(b.value)~s(x,df=bdf)+s(y,df=bdf),data=rgbImage)
...
p_5 <- plot(y ~ x, data=rgbImage, main="bluejay - gone, smoothing spline",
     col = rgb(rgbImage[c("r.value_ss", "g.value_ss", "b.value_ss")]),
     asp = 1, pch = 15, cex = 0.3)
p_5
...
```{r}
eps <- 0.2
rgbImage$r.value_ss_gone <- ifelse((abs(rgbImage[3] - rgbImage[6])>eps),1,rgbImage$r.value_ss)
rgbImage$g.value_ss_gone <- ifelse((abs(rgbImage[4] - rgbImage[7])>eps),1,rgbImage$g.value_ss)
rgbImage$b.value_ss_gone <- ifelse((abs(rgbImage[5] - rgbImage[8])>eps),1,rgbImage$b.value_ss)

p_6 <- plot(y ~ x, data=rgbImage, main="bluejay - gone, smoothing spline",
     col = rgb(rgbImage[c("r.value_ss_gone", "g.value_ss_gone", "b.value_ss_gone")]),
     asp = 1, pch = 15, cex = 0.3)
p_6
```
```{r}
...
p_7 <- plot(y ~ x, data=rgbImage, main="bluejay - gone, smoothing spline",
     col = rgb(rgbImage[c("r.value_ss_gone", "g.value_ss_gone", "b.value_ss_gone")]),
     asp = 1, pch = 15, cex = 0.3)
p_7
...
p_8 <- plot(y ~ x, data=rgbImage, main="bluejay - gone, smoothing spline",
     col = rgb(rgbImage[c("r.value_ss_gone", "g.value_ss_gone", "b.value_ss_gone")]),
     asp = 1, pch = 15, cex = 0.3)
p_8
